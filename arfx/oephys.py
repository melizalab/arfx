...
class continuous_dset(dataset):
    ...
    def __init__(self, base, structure):
        ...
        assert len(self.channel_attrs) == self.nchannels, "channel metadata doesn't match channel count"
        ...
        if data.size % self.nchannels != 0:
            raise IOError("size of file '%s' (%d) is not a multiple of the channel count (%d)" %
                          (datfile, data.size, self.nchannels))
        self.data = data.reshape((data.size // self.nchannels, self.nchannels))
        log.debug("- %s: array %s @ %.1f/s",
                  structure["folder_name"], self.data.shape, structure["sample_rate"])
...
class event_dset(dataset):
    ...
    def __init__(self, base, structure):
        ...
        if structure["type"] == "string":
            ...
            self.data = np.rec.fromarrays([timestamps, messages, channels], names=("start", "message", "channel"))
            self.units = ("samples", "", "")
            ...
            raise NotImplementedError("%s type event datasets not supported" % structure["type"])
        log.debug("- %s: array %s @ %.1f/s (compound type)",
                 structure["folder_name"], self.data.shape, structure["sample_rate"])
...
def script(argv=None):
    ...
    p.add_argument("-v", "--verbose", action="store_true", help="show verbose log messages")
    p.add_argument("--dry-run", action="store_true", help="load data but do not write anything to disk")

    p.add_argument("--skip-empty", action="store_true", help="skip empty datasets")
    p.add_argument("-n", help="name entries or files using %(metavar)s",
                   metavar="TEMPLATE", dest="template")
    p.add_argument("-k", metavar="KEY=VALUE", dest="attrs", action=core.ParseKeyVal,
                   help='specify attributes of entries')
    p.add_argument("-T", default=arf.DataTypes.UNDEFINED,
                   metavar="DATATYPE", dest="datatype", action=core.ParseDataType,
                   help="specify data type for continuous data (see --help-datatypes)")
    p.add_argument("-z", default=1, dest="compress", type=int,
                   help="set compression level in ARF (default: %(default)s)")

    p.add_argument("-f", required=True, metavar="FILE", dest="arffile",
    ...
    with arf.open_file(args.arffile, "a", **fp_args) as fp:
        ...
        for path in args.path:
            try:
                ...
                p.exit(-1)
            for structfile in glob.iglob(os.path.join(path, "**/structure.oebin"), recursive=True):
                dir = os.path.dirname(structfile)
                log.info("Reading from '%s':", dir)
                rec = recording(dir)

                rec_name = dir.replace("/", "_")
                entry = arf.create_entry(fp,
                                         name=rec_name,
                                         timestamp=timestamp,
